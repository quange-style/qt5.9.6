...
{
    qCDebug(qLcEvdevKey) << "Loading hikeymap " << file;
    ...
    if (!f.open(QIODevice::ReadOnly)) {
	 ...
	 char name[1024];
    if (ioctl(m_fd.get(), EVIOCGNAME(sizeof(name) - 1), name) >= 0) {
        qCDebug(qLcEvdevKey, "keyboard: %s: device name: %s", qPrintable(m_device), name);
    }
